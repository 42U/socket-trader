...
- **With the bridge AddOn**: on every connect, and once per day, the app asks NinjaTrader for the contract *it* would trade per root (`Instrument.GetInstrument` under NT's own rollover schedule) and caches the answer in config (`front_months`). Recompile the AddOn once to enable this — an older build simply refuses the query and the calendar takes over.
- **Without it**: a built-in calendar per product family — deliverable metals/ags roll a week before the delivery month (first-notice window), energy rolls off mid-way through its prior month, financials trade into the contract month like NT does. Whichever source names the **later** month wins, so a stale cache can never drag a signal backward.

Corrections only ever move **forward** and only on cataloged roots: a deliberate back-month signal and any exotic typed contract pass through untouched. Exits follow the roll — a rolled entry lives in the new month, so its closes must too — and every rolled `CLOSEPOSITION`/`REVERSEPOSITION` *also* sends plain closes for the **original** month in both contract sizes, so a position opened before the roll can never be stranded (NT no-ops whichever variants aren't held). Each roll is logged (`MONTH ROLL`) and flashed in the dashboard (`↷ ROLLED SI 09-26 → SI 12-26`). The same calendar drives the web ticket's contract picker, so manual orders stop offering dead months too.
...
1. **One position at a time — an entry RESETS the account, confirmed close-before-open.** When a signal is about to open a position on a prop account, **everything the account holds is closed first — the entry's own market included** — the close is **verified against NinjaTrader** (never assumed), and only then does the entry fire. If the close cannot be confirmed, the entry is **withheld** with a sticky alert — a missed trade is recoverable, a rule violation is not. Two strategies signalling two markets (your GC position, then an NQ entry) can never stack. Closing the same market too is deliberate: per the NT8 OIF docs `CLOSEPOSITION` cancels the instrument's working orders, but an opposite-direction fill merely **nets** the position — the old ATM's stop/target would stay live on a flat account and either could re-open it later — and a same-direction re-entry would stack contracts toward the firm's cap (the same cap-evasion pattern MFFU names a breach for micro/full twins). A `REVERSEPOSITION` is the one flow that keeps its own market — the reversal itself flips it — while every *other* market it holds is swept right behind it. **Just-written orders count as held**: an entry or reversal the router wrote that NinjaTrader hasn't shown as a position yet (a market order mid-fill, a resting limit entry) is tracked in-flight for 2 minutes, so a burst of signals seconds apart can't stack a second market; a limit entry older than that is invisible again until it fills.

   The whole decision is made **client-side, at signal time** — the publisher does not need to (and should not) send a precautionary close before an entry, and accounts *not* marked `prop` never have positions closed for them, so multi-market and multi-direction trading stays available on ordinary broker accounts. With the [live trade monitor](#optional-live-trade-monitor) streaming, the client already holds every account's position book when the signal lands: a **fresh book that shows nothing to close lets the entry fire immediately**, skipping the pre-entry NinjaTrader snapshot. Anything else — something to close, a stale or disconnected stream, an account missing from the dump, an outage-shaped `$0.00` balance — falls back to the confirmed snapshot path above; closes only ever fire off a fresh NinjaTrader dump, never off the cached book.
...
**Close-before-open without prop mode.** An ordinary broker account can opt into the same one-position-at-a-time entry behavior — the close-everything-then-place the publisher's server used to send — with `"close_before_open": true` in its profile. It gets the full entry engine (reset semantics, confirmed closes, the live-book fast path, in-flight tracking) but **none of prop's time rules** (no entry cutoff, no flat-by-close) and it is never swept by other accounts' cross-account hedge preemption — that ban is a prop-firm rule. Accounts with *neither* flag are never preemptively closed at all: that's how you run multi-market / multi-direction. One publisher-contract note for those unflagged accounts: a strategy that **flips direction must send `REVERSEPOSITION`** (and exits `CLOSEPOSITION`/`CLOSESTRATEGY`) — a bare opposite `PLACE` into a held position only nets it, leaving the old ATM bracket working.

```json
"account_profiles": {
  "Cash-001": { "close_before_open": true }
}
```
...
| `account_profiles` | Per-account trade profiles — allowed-symbols filter (`symbols_allowed`), size, contracts, direction, delay, stagger, ATM override, AI gate, the prop-firm flag (`prop`, `prop_firm`, `prop_flat_et`, `prop_cutoff_et` — see [Prop Firm Mode](#prop-firm-mode)), and `close_before_open` for one-position-at-a-time entries without prop's time rules |
