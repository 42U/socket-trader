...
|---|---|---|---|
| 1 | Intra-account hedge (same market, both sides) | Banned everywhere | Same-contract orders are left to NT netting (a partial exit stays an exit). A **micro/full twin** held against an entry is closed — both directions — before the entry fires, close confirmed |
| 2 | Concurrent markets (user's one-trade policy) | Not required by firms; safest common denominator | **Close-before-open**: every other-market position on the entering prop account is closed and **confirmed against NinjaTrader** before the entry is written. Unconfirmable close ⇒ entry withheld, sticky alarm |
